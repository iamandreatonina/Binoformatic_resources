...
We can see that there are a lot of DEGs, both up (right side of the plot) or down (left side of the plot) expressed.
```{r}
input_df<-DEGs
...
We can also represent the genes using a heatmap. A clustering process is operated. We plot only up or down expressed genes.
We can see that thanks to clusterization, chunks of expressed genes are outlined. 
the majority are up expressed genes. ????? che se faa
```{r}
...
heatmap(as.matrix(cpm_table[which(rownames(cpm_table) %in% DEGs$ensembl_gene_id[which(DEGs$class != '=')]),]),ColSideColors = col, cexCol = 0.5,
...
heatmap(as.matrix(cpm_table_log[which(rownames(cpm_table_log) %in% DEGs$ensembl_gene_id[which(DEGs$class != '=')]),]),ColSideColors = col, cexCol = 0.5,
        margins = c(4,4), col = pal, cexRow = 0.2) 
```
...
Perform Gene Ontology enrichment analysis (biologica process).
...
Then we proceed with the same processes but for down-regulated:
SO we create a new list of only the down regulated genes 
```{r}
...
We can use the library pathway and specifically the function pathway that generate a local cartoon of the pathway of interest underling the DEGs genes using specific colors. A measure of the DE is needed, here we use the Log Fold Change.
The pathway we represent is choose by looking at the top 10 expressed pathway from point 4, specifically we selected 'Vitamin D receptor pathway' for which we found in https://www.genome.jp/entry/hsa:7421 the corresponding pathway ID code hsa04928 for Parathyroid hormone synthesis, secretion and action.
```{r}
library(pathview)
logFC <-UPDegs$logFC
...
Then, we used DNAStringSet function to create DNAString object, needed to operate the motif enrichment.
We can now operate the enrichment and thanks to the function groupReport we obtain the TF enrichments across many sequences.
```{r}
...
```
0.9287791 is the result, indicating that the selected TF is able to bind almost all the promoters with a good affinity, as we expected since we chose the top enriched TF.


9. Use STRING database to find PPI interactions among differentially expressed genes and export the network in TSV format.

We created a .txt file containing all the gene_names of the up-regualated genes identified in the previous points. We then uploaded the file to the STRING tool to obtain and create a network. We finally downloaded the output in .tsv format, the file contain all the information on the edges found between the input genes (=our nodes).
```{r}
...
We first created the nodes annotations using bioMart. We filtered using the up-regulated genes names and selected as attributes the gene id, the description, the biotype, the start and end position, the chromosome name, and the strand.
